...
appcan.ready(function() {
    ...
    uexEventType.getDefaultInfo(dataBaseName,function(err,info){
        if(err==1){
...
function sendZhugeMessager() {
    ...
    appcan.window.evaluateScript('event_report', "utn_return('20')");
}
...
function initDomainInfo(){
    // return;
    var dbOperation = new DataBaseOperation();
    var sql = "select domainName,domainValue,domainCode,rowIndex from domain_table where domainName in ('pressure_grade', 'event_distance', 'construction_site', 'notification_type', 'protection_protocol') order by domainName,rowIndex";
    dbOperation.dbSelect(sql, function(err, data) {
        if (err != null) {
            ...
            // var ids = ['pressureGrade', 'eventDistance', 'constructionSite', 'notificationType', 'protectionProtocol'];
