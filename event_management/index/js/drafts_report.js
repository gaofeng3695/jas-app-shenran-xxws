    ...
    dbOperation.dbSelect(sql, function(err, result) {
        if (err == null) {
            ...
            if (eventData.category == "1") {
                $("#constructInfo").show();
                $("#handleEvent").show();
                pressureGrade = eventData.pressureGrade;
                eventDistance =eventData.eventDistance;
                notificationType =eventData.notificationType;
                protectionProtocol =eventData.protectionProtocol;
                contactPerson = eventData.contactPerson;
                contactNumber = eventData.contactNumber;

                $("#pressureGrade").val(pressureGrade);
                $("#eventDistance").val(eventDistance);
                $("#notificationType").val(notificationType);
                $("#protectionProtocol").val(protectionProtocol);
                $("#contactPerson").val(contactPerson);
                $("#contactNumber").val(eventData.contactNumber);
                category = 1;
            }else {
                $("#constructInfo").hide();
                $("#handleEvent").hide();
            ...
            if(status=="21"){
                ...
                $("#is_report .pay_list_c1").removeClass("on");
                $('.whether_no').css("color",'#ff3f3f');
                $("#is_report input[name='radio']").each(function(e){
                    if($(this).val()=='30'){
                        $(this).prop("checked",true);
                        $(this).closest('#is_report .pay_list_c1').addClass('on');
                    }
                });
                var userBo = JSON.parse(appcan.locStorage.getVal("userBo"));
...
appcan.ready(function() {
    ...
    // testAudio();
    $("#is_report  .pay_list_c1").each(function(e) {
        $(this).click(function() {
            $("#is_report .pay_list_c1").removeClass("on");
            $(this).addClass("on");
...
function sendZhugeMessager() {
    ...
    deleteOfflineData(formArry.objectId,function(){
         baseOperation.alertToast("数据提交完成！");
        if (appcan.locStorage.getVal("enterEventModel") != "") {
            appcan.window.evaluateScript('daily_check', 'checkObj.showMap(true)');
        }
        appcan.window.evaluateScript('drafts_report', "utn_return('90')");
    });
}
...
function on_submit(e) {
    ...
    // alert( JSON.stringify(people_data.map(function(item){
    //     return item.roleId;
    // }),0,4) )
    // if(people_data.map(function(item){
    //     return item.roleId;
    // }).indexOf('5aa6aead-64ae-4a38-acba-1917966638ca')==-1){
    //     baseOperation.alertToast("报送人中必须含有输配所长");
    ...
    var formArry2 = formOperation.getFormJson("#event_refer2");
    var desc = formArry.description;
    if(category == 1){
        
        // Object.assign(formArry, formArry2);
        var obj_temp = $.extend(formArry, formArry2);
        if (!validateNumber(formArry.contactNumber)) {
            appcan.window.evaluateScript('drafts_report', "changeButton()");
            return;
        };
        if (formArry.contactPerson == "") {
            baseOperation.alertToast("请输入联系人");
            appcan.window.evaluateScript('drafts_report', "changeButton()");
            return false;
        }
    }
    formArry.category = category;
    // Object.assign(formArry, domainNameObj);
    var obj_temp_1 = $.extend(formArry, domainNameObj);
    ...
    if (formArry.type == 0) {
        ...
        appcan.window.evaluateScript('drafts_report', "changeButton()");
        return false;
    } else if ($(".whether_people").text() == '') {
        baseOperation.alertToast("请选择报送人");
        appcan.window.evaluateScript('drafts_report', "changeButton()");
        return false;
    }
 ...
 */
function deleteOfflineData(eventid,cb) {
...
function initDomainInfo(){
    // return;
    var dbOperation = new DataBaseOperation();
    var sql = "select domainName,domainValue,domainCode,rowIndex from domain_table where domainName in ('pressure_grade', 'event_distance',  'notification_type', 'protection_protocol') order by domainName,rowIndex";
    dbOperation.dbSelect(sql, function(err, data) {
        if (err != null) {
            baseOperation.alertToast("加载异常");
        } else {
            $("#pressureGrade").html("");
            $("#eventDistance").html("");
            // $("#constructionSite").html("");
            $("#notificationType").html("");
            $("#protectionProtocol").html("");
            var pressure_grade_html="";
            var event_distance_html="";
            // var construction_site_html="";
            var notification_type_html="";
            var protection_protocol_html="";
            data.forEach(function(item,index){
                if (item.domainName == "pressure_grade") {
                    pressure_grade_html +=(item.rowIndex%2==1?'<div class="ub uof whether_list">':'')+
                    '<div class="ub whether_fl ub-ac" style="width:50%">'+
                        '<span class="pay_list_c1 '+(item.rowIndex==1?"on":"")+'">'+
                            '<input type="radio" id="pressureGrade_'+item.rowIndex+'" '+(item.rowIndex==1?"checked=\"checked\"":"")+' form="event_refer2" name="pressureGrade" value="'+item.domainCode+'" class="radioclass">'+
                        '</span>'+
                        '<label class="" for="pressureGrade_'+item.rowIndex+'">'+item.domainValue+'</label>'+
                    '</div>'+
                    (item.rowIndex%2==0?'</div>':'');
                    if(item.rowIndex==1){
                        domainNameObj["pressureGradeName"]=item.domainValue
                    }
                }else if(item.domainName == "event_distance"){
                    event_distance_html +=
                    '<div class="ub whether_fl ub-ac">'+
                        '<span class="pay_list_c1 '+(item.rowIndex==1?"on":"")+'">'+
                            '<input type="radio" id="eventDistance_'+item.rowIndex+'" '+(item.rowIndex==1?"checked=\"checked\"":"")+' form="event_refer2" name="eventDistance" value="'+item.domainCode+'" class="radioclass">'+
                        '</span>'+
                        '<label class="" for="eventDistance_'+item.rowIndex+'">'+item.domainValue+'</label>'+
                    '</div>';
                    if(item.rowIndex==1){
                        domainNameObj["eventDistanceName"]=item.domainValue
                    }
                // }else if(item.domainName == "construction_site"){
                    // construction_site_html +=(item.rowIndex%2==1?'<div class="ub uof whether_list">':'')+
                    // '<div class="ub whether_fl ub-ac" style="width:50%">'+
                        // '<span class="pay_list_c1 '+(item.rowIndex==1?"on":"")+'">'+
                            // '<input type="radio" id="constructionSite_'+item.rowIndex+'" '+(item.rowIndex==1?"checked=\"checked\"":"")+' form="event_refer2" name="constructionSite" value="'+item.domainCode+'" class="radioclass">'+
                        // '</span>'+
                        // '<label class="" for="constructionSite_'+item.rowIndex+'">'+item.domainValue+'</label>'+
                    // '</div>'+
                    // (item.rowIndex%2==0?'</div>':'');
                    // if(item.rowIndex==1){
                        // domainNameObj["constructionSiteName"]=item.domainValue
                    // }
                }else if(item.domainName == "notification_type"){
                    notification_type_html +=
                    '<div class="ub whether_fl ub-ac">'+
                        '<span class="pay_list_c1 '+(item.rowIndex==1?"on":"")+'">'+
                            '<input type="radio" id="notificationType_'+item.rowIndex+'" '+(item.rowIndex==1?"checked=\"checked\"":"")+' form="event_refer2" name="notificationType" value="'+item.domainCode+'" class="radioclass">'+
                        '</span>'+
                        '<label class="" for="notificationType_'+item.rowIndex+'">'+item.domainValue+'</label>'+
                    '</div>';
                    if(item.rowIndex==1){
                        domainNameObj["notificationTypeName"]=item.domainValue
                    }
                }else if(item.domainName == "protection_protocol"){
                    protection_protocol_html +=(item.rowIndex%2==1?'<div class="ub uof whether_list">':'')+
                    '<div class="ub whether_fl ub-ac" style="width:50%">'+
                        '<span class="pay_list_c1 '+(item.rowIndex==1?"on":"")+'">'+
                            '<input type="radio" id="protectionProtocol_'+item.rowIndex+'" '+(item.rowIndex==1?"checked=\"checked\"":"")+' form="event_refer2" name="protectionProtocol" value="'+item.domainCode+'" class="radioclass">'+
                        '</span>'+
                        '<label class="" for="protectionProtocol_'+item.rowIndex+'">'+item.domainValue+'</label>'+
                    '</div>'+
                    (item.rowIndex%2==0?'</div>':'');
                    if(item.rowIndex==1){
                        domainNameObj["protectionProtocolName"]=item.domainValue
                    }
                }
            });
            $("#pressureGrade").append(pressure_grade_html);
            $("#eventDistance").append(event_distance_html);
            // $("#constructionSite").append(construction_site_html);
            $("#notificationType").append(notification_type_html);
            $("#protectionProtocol").append(protection_protocol_html);
            var ids = ['pressureGrade', 'eventDistance', 'notificationType', 'protectionProtocol'];
            initData();
            addRadionListener(ids);
        }
    });
}
 ...
 */
function addRadionListener(htmlIds){
    htmlIds.forEach(function(id,index){
        $("#"+id+" .pay_list_c1").each(function(e) {
            $(this).click(function() {
                 try{
                    $("#"+id+" .pay_list_c1").removeClass("on");
                    $(this).addClass("on");
                    $(this).find("input[type='radio']").prop("checked", true);
                    var value = $(this).find("input[type='radio']").prevObject.context.parentNode.innerText;
                    domainNameObj[id+"Name"]=value;
                 }catch(e){
                     alert(e);
                 }
            })
...
function initRadioValue(eventData){
    var ids = ['pressureGrade', 'eventDistance',  'notificationType', 'protectionProtocol'];
    ids.forEach(function(id,index){
        $("#"+id+" .pay_list_c1").removeClass("on");
        var value = eventData[id];
        domainNameObj[id+"Name"] = eventData[id+"Name"]
        $("#"+id+" input[name='"+id+"']").each(function(e){
            if($(this).val()==value){
                $(this).prop("checked",true);
                $(this).closest('#'+id+' .pay_list_c1').addClass('on');
            }
        }); 
    });
}
...
function selectEventType(){
    try{
        var dataBaseName = (new DataBaseOperation()).dbGetBaseName();
        var param = {
            "dataBaseName":dataBaseName,
            "objectId": selectTypeVal
        };
        uexEventType.open(JSON.stringify(param),function(err,info){
            if(err==1){
                var data = JSON.parse(info);
                $("#select_type").val(data.objectId);
                $("#select_type_name").val(data.parentName+"-"+data.typeName);
                category = data.category;
                // 判断类型是否为第三方施工，若是则显示施工信息及事件处理
                if(category == 1){
                    $("#constructInfo").show();
                    $("#handleEvent").show();
                    
                    if(pressureGrade == ""){
                        $("#pressureGrade_1").parent().addClass("on");
                        $("#pressureGrade_1").attr("checked",'checked');
                    }else {
                        $("#pressureGrade").val(pressureGrade);
                    }
                    if(eventDistance == ""){
                        $("#eventDistance_1").parent().addClass("on");
                        $("#eventDistance_1").attr("checked",'checked');
                    }else {
                        $("#eventDistance").val(eventDistance);
                    }
                    if(notificationType == ""){
                        $("#notificationType_1").parent().addClass("on");
                        $("#notificationType_1").attr("checked",'checked');
                    }else {
                        $("#notificationType").val(notificationType);
                    }
                    if(protectionProtocol == ""){
                        $("#protectionProtocol_1").parent().addClass("on");
                        $("#protectionProtocol_1").attr("checked",'checked');
                    }else {
                        $("#protectionProtocol").val(protectionProtocol);
                    }
                    if (contactPerson != "") {
