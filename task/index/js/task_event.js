...
function initEventData(){
    ...
    jasHttpRequest.jasHttpGet(partURL,function(id, state, dbSource){
        ...
        if(resultData.success=="1"){
            ...
            $(".status").text(data_all[0].eventStatusCn);  
            if (data_all[0].category == 1) {
                $("#constructInfo").show();
                $("#handleEvent").show();
                $(".pressure_grade").text(data_all[0].pressureGradeValue);       //事件时间
                $(".event_distance").text(data_all[0].eventDistanceValue);       //事件时间
                $(".notification_type").text(data_all[0].notificationTypeValue);       //事件时间  
                $(".protection_protocol").text(data_all[0].protectionProtocolValue);       //事件时间 
                $(".contactPerson").text(data_all[0].contactPerson);  
                $(".contactNumber").text(data_all[0].contactNumber);  
            }else {
                $("#rectificationDiv").show();
                $(".rectificationDate").text(data_all[0].rectificationDate);  
                
            }
            appcan.locStorage.val('eventStatusCn',data_all[0].eventStatusCn);
            appcan.locStorage.val('eventStatusCode',data_all[0].eventStatusCode);
