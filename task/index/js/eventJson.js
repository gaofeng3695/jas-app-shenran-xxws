...
//                appcan.openWinWithUrl("event_details","../event_details.html");
    ...
    disposal : function(data,id){      //获取处置详情信息
        ...
        queryResult.forEach(function (item) {
            var params ={
                "objectId": item.objectId,                //处置信息唯一标识
                "content": item.content,        //事件描述
                "createUserName": item.createUserName,       //处置人
                "disposeValue": item.disposeValue,     //处置类型拼接中文
                "typeCode":item.typeCode,         //处置类型
                "pic": item.pic,
                "createTime":item.createTime,             //处理时间
                "recevieUserName":item.recevieUserName,             //接收人
                "audio":item.audio,
                "eventStatus": item.eventStatus,           // 工地状态
                "rectificationDate": item.rectificationDate    //整改时间
