...
appcan.ready(function() {
    ...
    $("#rectificationDate").val((new Date()).Format("yyyy-MM-dd"));
    ...
    var eventStatusCn = appcan.locStorage.getVal('eventStatusCn');
    var eventStatusCode = appcan.locStorage.getVal('eventStatusCode');
    if (eventStatusCode.indexOf("C") > -1) {
        $("#eventStatusDiv").show();
        if (eventStatusCode.indexOf("04") > -1) {
            $("#active_parent").removeClass("on");
            $("#stable_parent").addClass("on");
            $("#stable").attr("checked",'checked');
            $("#active").removeAttr("checked");
        }else{
            $("#active_parent").addClass("on");
            $("#stable_parent").removeClass("on");
            $("#active").attr("checked",'checked');
            $("#stable").removeAttr("checked");
        };
    }else{
        $("#rectificationDateDiv").show();
    };
...
function on_submit() {
    ...
    var eventStatusCode = appcan.locStorage.getVal('eventStatusCode');
    if (eventStatusCode.indexOf("C") > -1) {
        formArry.rectificationDate = "";
    }else{
        formArry.eventStatus = "";
    };
        ...
        if (dbSource.length == 0) {
            ...
            if (resultData.success == "1") {
                ...
                if (taskState[0].status == 21) {
                    ...
                    /*信息类型为邀请时，必须选择接收人，特此说明*/
                    if (formArry.type == "20") {
                        if ($("#recipient").val() == "") {
                            baseOperation.alertToast("请选择【邀请】消息的接收人！");
                            appcan.window.evaluateScript('disposal_report', "changeButton()");
                            return;
                        }
                    }
...
function initDateRange() {
    ...
    $("#rectificationDate").mobiscroll().datetime({
        theme : theme,
        lang : "zh",
        display : "center",
        //min: new Date(now.getFullYear(), now.getMonth(), now.getDate()),
...
function showDataTime() {
    ...
    inst.show();
    var rectificationDate = mobiscroll.date('#rectificationDate', {
        theme : theme,
        lang : "zh",
        display : "center",
        min : new Date(),
