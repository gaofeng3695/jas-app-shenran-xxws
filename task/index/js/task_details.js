...
function isprivilegeCloseTask(){
     var privilegeRole = ['5aa6aead-64ae-4a38-acba-1917966638ca', 'b5215de6-12f6-4ebc-a349-db00dccb96f2'];
     var userBo=JSON.parse(appcan.locStorage.getVal("userBo"));
     if(!(privilegeRole.indexOf(userBo.roleIds)>-1)){//此时不可以进行关闭  进行蒙层 并且不可以点击
        $(".event_delete").removeAttr("onclick");
        $(".event_delete").addClass("nonePrivilege");
        $(".event_report").removeAttr("onclick");
        $(".event_report").addClass("nonePrivilege");
     }
}
