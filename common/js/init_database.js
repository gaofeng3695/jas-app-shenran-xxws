var initAppDB = {
    ...
    createCustomEventType: function() {
        ...
        //!必须有
        var dbOperation = new DataBaseOperation();
        var sql = "create table if not exists customEventType (id INTEGER PRIMARY KEY AUTOINCREMENT,parentId text,objectId text,typeName text,parentIndexNum int,indexNum int,iconName text,category text);";
        dbOperation.dbCreateTable(sql, function(err, data) {
            if (err == null && data >= 0) {
    ...
    initSysConfig: function() {
        ...
        dbOperation.dbSelect("select * from IpConfig", function(err, data) {
            if (err != null) {
                ...
                appcan.locStorage.setVal('serverIP', 'srtzgw.xunxianweishi.com');
            ...
            if (err == null && data == "") {
                ...
                appcan.locStorage.setVal('serverIP', 'srtzgw.xunxianweishi.com');
                appcan.locStorage.setVal('serverPort', '');
                dbOperation.dbExec("INSERT INTO IpConfig (protocol,ip,port) VALUES ('https://','srtzgw.xunxianweishi.com','');", function(err2, data2) {});
                return;
            }
    ...
    initTable:function(){
        var init_sql=[
            ...
            "create table if not exists customEventType (id INTEGER PRIMARY KEY AUTOINCREMENT,parentId text,objectId text,typeName text,parentIndexNum int,indexNum int,iconName text,category text);",
