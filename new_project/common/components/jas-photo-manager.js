...
Vue.component('jas-photo-manager', {
  ...
  methods: {
    requestFile: function (bizId) {
      var that = this;
      that._bizId = bizId;
      var url = "cloudlink-core-file/attachment/getFileIdListByBizIdAndBizAttr";
    ...
    _deleteFilesToServer: function (cb) {
      ...
      var nDone = 0;
      var nAll = that.filesToBeDelete.length;
      that.filesToBeDelete.forEach(function (item, index) {
        jasTools.ajax.post("cloudlink-core-file/attachment/deleteByBizIdAndBizAttrAndFileId", {
          "businessId": that._bizId,
          "bizType": "file",
          "fileId": item,
        }, function (data) {
          nDone++;
    ...
    uploadFiles: function (sBizId, cbsuccess, cbfail) {
      var that = this;
    ...
    styleBgImg: function (item) {
      ...
      if (item.oid && !item.src) {
        ...
        var src = jasTools.base.setParamsToUrl(jasTools.ajax.serverURL + 'cloudlink-core-file/file/downLoad', {
          fileId: item.oid
        });
        // var src = [
        //   jasTools.ajax.serverURL,
        //   // 'DAQProject/attachment/download.do?oid=',
        //   '/attachment/deleteByBizIdAndBizAttrAndFileId?oid=',
        //   item.oid,
        //   '&token=',
